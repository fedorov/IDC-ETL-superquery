...
WITH segs AS (
  SELECT
    ...
    SegmentSequence
  FROM
    `idc-dev-etl.pre_mvp_temp.dicom_all` # Debug:
...
SELECT
  ...
  CASE
    ...
    ELSE NULL
  END AS AnatomicRegionModifier,
  CASE
    ARRAY_LENGTH(unnested.SegmentedPropertyCategoryCodeSequence)
    WHEN 0 THEN NULL
    ELSE unnested.SegmentedPropertyCategoryCodeSequence [
  OFFSET
    (0)]
  END AS SegmentedPropertyCategory,
  CASE
    ARRAY_LENGTH(unnested.SegmentedPropertyTypeCodeSequence)
    WHEN 0 THEN NULL
    ELSE unnested.SegmentedPropertyTypeCodeSequence [
  OFFSET
    (0)]
  END AS SegmentedPropertyType,
...
FROM
  segs
  CROSS JOIN UNNEST(SegmentSequence) AS unnested # correctness check: there should be 4 segmented nodules for this subject
